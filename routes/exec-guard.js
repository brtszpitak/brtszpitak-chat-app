...
function isPaused() {
  return fs.existsSync(AUTONOMY_FLAG);
}
...
function execAllowed(cmd, args) {
  const entry = EXEC_ALLOW.binaries.find(
    (b) => b.cmd.toLowerCase() === String(cmd).toLowerCase(),
  );
...
export default function execGuard(req, res, next) {
  ...
  if (maybePathArg) {
    ...
    if (POLICY.exec.requireVerifiedSource) {
      try {
        const manifest = fs
          .readFileSync(path.join(DL_ROOT, 'manifest.jsonl'), 'utf8')
          .trim()
          .split(/\r?\n/)
          .filter(Boolean)
          .map(JSON.parse);
        ...
        const entry = manifest
          ...
          .find(
            (e) =>
              path.resolve(e.path) === abs && e.outcome === 'OK' && e.sha256,
          );
        if (!entry)
          return res
            .status(403)
            .json({ error: 'EXEC_NOT_VERIFIED_IN_MANIFEST' });
      } catch {
