      ...
      if (evt.delta) {
        ...
        setMsgs((m) => {
          ...
          if (last?.role === 'assistant') {
            const mm = [...m];
            mm[mm.length - 1] = { role: 'assistant', content: reply };
            return mm;
          }
    ...
    <div className="p-4 max-w-3xl mx-auto space-y-4">
      ...
      <div className="flex gap-6 items-center">
        ...
        <div className="text-sm opacity-70">
          <p>Streaming chat with voice. Toggle auto-speak below.</p>
        </div>
      </div>
      <div className="space-y-2 h-[45vh] overflow-auto border rounded p-3 bg-white/60">
        {msgs.map((m, i) => (
          <div key={i} className={m.role === 'user' ? 'text-right' : ''}>
            <span className="rounded px-3 py-2 inline-block border bg-white">
              {m.content}
            </span>
          </div>
        ))}
      ...
      <div className="flex gap-2">
        ...
        />
        <button className="border rounded px-3 py-2" onClick={startSTT}>
          🎤
        </button>
        <button className="border rounded px-3 py-2" onClick={send}>
          Send
        </button>
      </div>
