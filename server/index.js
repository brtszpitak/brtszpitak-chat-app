...
app.use((req, _res, next) => { /* tiny log */ next(); });
  ...
  stack.forEach(function (layer) {
    if (layer && layer.route && layer.route.path && layer.route.methods) {
      const methods = Object.keys(layer.route.methods)
        ...
        .join(",");
      list.push(methods + " " + layer.route.path);
    }
  });
  res.json(list);
});
...
// == Auto-load secure /download route ==
import('./downloadRoute.mjs').then(m => m.registerDownloadRoute(app)).catch(e => console.error('download route load failed', e));

// Simple health endpoint for Electron/dev
app.get('/health', (req,res)=>res.json({ok:true}));
