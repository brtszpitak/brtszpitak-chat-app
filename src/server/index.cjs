...
app.post('/chat', (req, res) => {
  ...
  logLine(
    `/chat | ${String(msg).slice(0, 200).replace(/\s+/g, ' ') || '(no message)'}`,
  );
...
function runCmd(cmd) {
  return new Promise((resolve, reject) => {
    if (!ALLOW_CMDS.some((rx) => rx.test(cmd)))
      return reject(new Error(`blocked cmd: ${cmd}`));
    logLine(`run | ${cmd}`);
  ...
  try {
    for (const t of tasks) {
      if (t.kind === 'cmd') {
        await runCmd(t.cmd);
        res.write(
          JSON.stringify({ type: 'delta', content: `STATUS: ran ${t.cmd}` }) +
            '\n',
        );
      } else if (t.kind === 'write') {
        await writeFileSafe(t.rel, t.content ?? '');
        res.write(
          JSON.stringify({ type: 'delta', content: `STATUS: wrote ${t.rel}` }) +
            '\n',
        );
      } else {
