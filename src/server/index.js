...
app.post("/chat", (req, res) => {
  ...
  const role = body.role || (Array.isArray(body.messages) ? (body.messages.at(-1)?.role || "user") : "user");

  const logDir = path.join(process.cwd(), "logs");
  if (!fs.existsSync(logDir)) fs.mkdirSync(logDir, { recursive: true });
  const stamp = new Date().toISOString().replace("T"," ").slice(0,19);
  const logPath = path.join(logDir, `autonomy-${stamp.slice(0,10).replace(/-/g,"")}.log`);
  const summary = text ? text.replace(/\s+/g," ").slice(0,300) : "(no message)";
