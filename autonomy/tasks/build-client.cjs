...
function looksLikeVite(client) {
  if (
    hasFile(path.join(client, 'vite.config.ts')) ||
    hasFile(path.join(client, 'vite.config.js'))
  )
    return true;
...
async function buildClient(client) {
  ...
  if (scripts.build) {
    ...
    throw new Error(
      'No known build method (no scripts.build, no Vite/CRA/Next detected).',
    );
  }

  // Verify output folder (try common names)
  const candidates = ['dist', 'build', '.next', 'out'].map((d) =>
    path.join(client, d),
  );
  const found = candidates.find(hasDir);
  if (!found)
    throw new Error(
      'Client build finished but no dist/build/.next/out folder found',
    );
