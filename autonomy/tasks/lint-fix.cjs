...
async function ensurePrettier(cwd) {
  const hasLocal = fs.existsSync(path.join(cwd, "node_modules", ".bin", "prettier"));
  if (!hasLocal) {
...
async function maybeEnsureESLint(cwd) {
  const hasLocal = fs.existsSync(path.join(cwd, "node_modules", ".bin", "eslint"));
  if (!hasLocal && fs.existsSync(path.join(cwd, "package.json"))) {
    const pkg = JSON.parse(fs.readFileSync(path.join(cwd, "package.json"), "utf8"));
    const need = (pkg.devDependencies && pkg.devDependencies.eslint) ||
...
async function runESLintFix(cwd) {
  if (!fs.existsSync(path.join(cwd, "node_modules", ".bin", "eslint"))) return false;
  ...
  run: async () => {
    ...
    for (const cwd of roots) {
      try {
        ...
        notes.push(`${path.basename(cwd) || "root"}: prettier=${pOk}, eslintFix=${eOk}`);
      } catch (e) {
