...
module.exports = {
  ...
  run: async ({ exec, proposeDiff, datetime }) => {
    ...
    const plan = await proposeDiff({ goals: [
      "improve reliability", "reduce warnings", "enhance logs"
    ...
    try { await exec("npm", ["run","lint","--","--max-warnings=0"], { cwd: process.cwd() }); } catch(e){}
    const client = path.resolve(process.cwd(), "client");
    try { await exec("npm", ["run","build"], { cwd: client }); } catch(e){}

    await git.addAll();
    await git.commit(`self-rewrite: ${applied} edits`);

    return { ok: true, note: `self-rewrite committed ${applied} edits on ${branch}` };
  }
};
