...
function trySh(cmd, args = [], cwd = process.cwd()) {
  try { return sh(cmd, args, cwd); } catch { return ""; }
}
...
function ensureLocalIdentity(cwd) {
  ...
  if (!email) trySh("git", ["config", "user.email", "alice@example.local"], cwd);
}
...
module.exports = {
  ...
  run: async ({ exec, proposeDiff, datetime }) => {
    ...
    } catch { /* ignore proposeDiff issues */ }
    ...
    try {
      sh("git", ["-c","commit.gpgsign=false","commit","-m", msg, "--no-verify"], cwd);
      return { ok: true, note: `self-rewrite committed (${msg}) on ${branch}` };
    } catch (e) {
      const head = trySh("git", ["rev-parse", "--abbrev-ref", "HEAD"], cwd);
      const stagedArr = trySh("git", ["diff", "--cached", "--name-only"], cwd).split(/\r?\n/).filter(Boolean);
      ...
      return {
        ...
        note:
          ...
          "branch=" + String(head) +
          " staged=" + JSON.stringify(stagedArr.slice(0, 5)) +
          ' msg="' + String(msg) + '" ' +
          "err=" + errStr,
      };
    }
