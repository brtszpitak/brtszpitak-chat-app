...
async function hasWorkingChanges(execFn, cwd) {
  try {
    const txt = await textOut(runExec(execFn, "git", ["status", "--porcelain"], { cwd }));
    return txt.length > 0;
  } catch { return true; }
}
...
async function stagedList(execFn, cwd) {
  try {
    return await textOut(runExec(execFn, "git", ["diff", "--cached", "--name-only"], { cwd }));
  } catch { return ""; }
}
...
async function ensureLocalIdentity(execFn, cwd) {
  ...
  if (!email) await runExec(execFn, "git", ["config", "user.email", "autonomy@local"], { cwd });
  if (!name)  await runExec(execFn, "git", ["config", "user.name", "Alice Autonomy"], { cwd });
}
...
module.exports = {
  ...
  run: async ({ exec, proposeDiff, datetime }) => {
    ...
    // best-effort hygiene
    try { await runExec(exec, "npx", ["prettier", "--write", "."], { cwd: process.cwd() }); } catch {}
    try { await runExec(exec, "npm", ["run", "lint", "--", "--max-warnings=0"], { cwd: process.cwd() }); } catch {}
    try { await runExec(exec, "npm", ["run", "build"], { cwd: path.resolve(process.cwd(), "client") }); } catch {}

    // stage & commit (safe)
    const cwd = process.cwd();
    try {
      if (!(await hasWorkingChanges(exec, cwd))) {
        return { ok: true, note: `self-rewrite: no changes; repo clean on ${branch}` };
      }
      ...
      return { ok: false, note: `self-rewrite: unexpected error: ${e && e.message ? e.message : e}` };
    }
  },
};
